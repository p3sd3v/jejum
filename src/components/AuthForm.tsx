...
import { ChromeIcon } from 'lucide-react'; // Placeholder for Google Icon
...
const AuthForm: React.FC<AuthFormProps> = ({ mode }) => {
  ...
  return (
    <Card className="w-full max-w-md shadow-xl">
      <CardHeader>
        <CardTitle className="text-3xl font-headline">{mode === 'login' ? 'Bem-vindo de Volta!' : 'Crie sua Conta'}</CardTitle>
        <CardDescription>{mode === 'login' ? 'Faça login para continuar no JejumZen.' : 'Junte-se ao JejumZen para começar sua jornada.'}</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <Button variant="outline" className="w-full" onClick={handleGoogleSignIn} disabled={isLoading}>
          {/* Using a simple SVG for Google Icon as lucide-react might not have a direct brand icon */}
      ...
      <CardFooter className="flex justify-center">
        <p className="text-sm text-muted-foreground">
          ...
          <Button variant="link" className="text-accent" onClick={() => router.push(mode === 'login' ? '/signup' : '/login')}>
